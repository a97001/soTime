    ...
    return {
      ...
      allMyEvents: (req, res, next) => {
        co(function*() {
          ...
          if (req.query.type) {
            myEvents = yield Event.find({host: me._id, group: null, friendship: null, startTime: {$lte: new Date(req.query.to)}, endTime: {$gte: new Date(req.query.from)}, type: req.query.type}, 'title description type startTime allDay endTime venue isPublic').lean().exec();
          } else {
            myEvents = yield Event.find({host: me._id, group: null, friendship: null, startTime: {$lte: new Date(req.query.to)}, endTime: {$gte: new Date(req.query.from)}}, 'title description type startTime allDay endTime venue isPublic').lean().exec();
          }
      ...
      allFriendshipEvents: (req, res, next) => {
        co(function*() {
          ...
          if (req.query.type) {
            friendshipEvents = yield Event.find({friendship: req.friendship._id, startTime: {$lte: new Date(req.query.to)}, endTime: {$gte: new Date(req.query.from)}, type: req.query.type}, 'title description type startTime allDay endTime venue isPublic').lean().exec();
          } else {
            friendshipEvents = yield Event.find({friendship: req.friendship._id, startTime: {$lte: new Date(req.query.to)}, endTime: {$gte: new Date(req.query.from)}}, 'title description type startTime allDay endTime venue isPublic').lean().exec();
          }
